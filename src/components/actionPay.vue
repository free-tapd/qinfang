    ...
    <div class="cover" v-show="isClose">
      <div class="pay-pannel">
        ...
        <ul>
          ...
          <li class="flex-r vux-1px-b" v-for="(v,i) in payMethods" :key="i" @click="getItem(v,i)">
            <div class="pay-methods flex-r">
              <img :src="v.icon" alt>
              <div class="pay-methods-text">
                <p>{{v.title}}</p>
        ...
        <!-- 立即支付 -->
        <div class="pay-btn-box flex-r" @click="changeJump('./successAppointment',{})">
          <div class="pay-btn">确认支付￥{{totalPay}}</div>
        </div>
      </div>
    </div>
  </div>
